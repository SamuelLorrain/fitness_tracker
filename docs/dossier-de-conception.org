...
#+ATTR_HTML: :width 300px
   ...
   3. Une architecture en couche, inspirée par ~Clean Architecture~:
      Il y a une couche ~Domaine~, qui décrit les ~Entités~ et ~Aggregats~ du systême, une couche ~Infrastrucutre~,
...
- La couche /Domaine/ contient les /Entités/ et /Aggregats/, soit, les objets domaine de l'application. Dans notre cas,
...
- La couche /Application/ ou /Service/ a pour fonction de décrire des opérations concrètes sur les /Entités/, et de
...
On peut voir que la couche /Domaine/, et l'entité /Food/ ne dépendent de rien, on peut ainsi modifier
...
*** Domaine
Voici une vue d'ensemble des entités de l'application:
...
|----------------+-----------------------------------------------------------------|
|            409 | Renvoyé lorsqu'on essaye de créer une entité qui existe        |
...
|        |                     | chercher les entités par leur nom.                                    |
|        |                     | Les entités ainsi retournées sont une version tronquée de l'entité   |
|        |                     | /Food/ par soucis de performance.                                      |
|--------+---------------------+------------------------------------------------------------------------|
| GET    | ~/food/{food_uuid}~ | Permet d'obtenir une entité /Food/ par sont ~UUID~.                   |
|--------+---------------------+------------------------------------------------------------------------|
| POST   | ~/food/~            | /Endpoint/ pour créer une nouvelle entité de type /Food/.             |
|--------+---------------------+------------------------------------------------------------------------|
| DELETE | ~/food/{food_uuid}~ | Permet de supprimer une entité /Food/, les utilisateurs ne peuvent    |
|        |                     | supprimer uniquement les entités qu'ils ont créé eux même.            |
|--------+---------------------+------------------------------------------------------------------------|

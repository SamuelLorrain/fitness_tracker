      ...
      <IonContent className="ion-padding">
        <form onSubmit={tdeeFormik.handleSubmit}>
          ...
          <IonInput
            ...
            onIonChange={(e) =>
              tdeeFormik.setFieldValue("weight", e.detail.value)
            }
          ...
          <IonInput
            ...
            onIonChange={(e) =>
              tdeeFormik.setFieldValue("height", e.detail.value)
            }
          ...
          <IonSelect
            ...
            onIonChange={(e) =>
              tdeeFormik.setFieldValue("activity", e.detail.value)
            }
          ...
          >
            ...
            <IonSelectOption value="light">
              Light Exercice (1-2days/week)
            </IonSelectOption>
            <IonSelectOption value="moderate">
              Moderate Exercice (3-5days/week)
            </IonSelectOption>
            <IonSelectOption value="heavy">
              Heavy Exercice (6-7days/week)
            </IonSelectOption>
            <IonSelectOption value="athlete">
              Athlete (2x per day)
            </IonSelectOption>
          </IonSelect>
        </form>
          ...
          {tdeeData ? (
            <>
              <div className="margin-bottom-small">
                TDEE (maintenance) : {tdeeData.getTDEE().toFixed(0)} kcal/day
              </div>
              <div className="margin-bottom-small">
                Bulk : {(tdeeData.getTDEE() * 1.1).toFixed(0)} kcal/day
              </div>
              <div className="margin-bottom-small">
                Cut : {(tdeeData.getTDEE() * 0.9).toFixed(0)} kcal/day
              </div>
              <IonButton
                expand="full"
                onClick={() => updateGoals(tdeeData.getTDEE().toFixed(0))}
              >
                Set goal to maintenance
              </IonButton>
              <IonButton
                expand="full"
                onClick={() =>
                  updateGoals((tdeeData.getTDEE() * 1.1).toFixed(0))
                }
              >
                Set goal to bulk
              </IonButton>
              <IonButton
                expand="full"
                onClick={() =>
                  updateGoals((tdeeData.getTDEE() * 0.9).toFixed(0))
                }
              >
                Set goal to cut
              </IonButton>
